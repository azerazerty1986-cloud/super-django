/* ================================================================== */
/* ===== [04] الملف: 04-telegram.js - نظام تلغرام متكامل ===== */
/* ===== مع نظام رمز التحقق للمدير (24 حرف + رقم) ===== */
/* ================================================================== */
...
// ===== [4.3] نظام رمز التحقق للمدير =====
let pendingAdminVerification = {
    userData: null,
    code: null,
    expiresAt: null
};

// ===== [4.3.1] إنشاء رمز تحقق عشوائي (24 حرف + رقم) =====
...
// ===== [4.3.2] إرسال رمز التحقق إلى قناة التلغرام =====
async function sendVerificationCodeToTelegram(userData, code) {
    try {
        const message = `#admin_verification 🔐 *طلب تسجيل مدير جديد*
━━━━━━━━━━━━━━━━━━━━━━
👤 *الاسم:* ${userData.name}
...
⚠️ *هذا الرمز صالح لمدة 5 دقائق فقط*
📝 *قم بنسخ الرمز وإدخاله في التطبيق*

✅ *بعد التحقق، ستحصل على صلاحيات كاملة كمدير*`;
        ...
        if (data.ok) {
            console.log(`✅ تم إرسال رمز التحقق إلى القناة`);
...
function showVerificationModal() {
    ...
    modal.innerHTML = `
        <div style="background: var(--bg-secondary);
            ...
            <h2 style="color: var(--gold); margin-bottom: 15px;">تأكيد تسجيل المدير</h2>
            <p style="color: var(--text-secondary); margin-bottom: 10px;">
                تم إرسال رمز التحقق إلى قناة التلغرام
            </p>
            <p style="color: var(--text-secondary); font-size: 12px; margin-bottom: 25px;">
                الرمز صالح لمدة 5 دقائق
            </p>
            
            <div style="margin-bottom: 25px;">
                <label style="display: block; margin-bottom: 10px; color: var(--text);">
                    أدخل رمز التحقق (24 حرف + رقم)
                </label>
            ...
            <div id="timerDisplay" style="margin-top: 20px; font-size: 12px; color: var(--text-secondary);">
                ⏰ الوقت المتبقي: <span id="timer" style="color: var(--gold); font-weight: bold;">5:00</span>
            </div>
        </div>
    `;
    ...
    document.getElementById('verifyCodeBtn').onclick = () => {
        ...
        verifyAndCompleteAdminRegistration(enteredCode);
    };
}
...
function startTimer() {
    ...
    timerInterval = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            if (pendingAdminVerification.code) {
                pendingAdminVerification = { userData: null, code: null, expiresAt: null };
                showNotification('⏰ انتهت صلاحية رمز التحقق، يرجى المحاولة مرة أخرى', 'warning');
            }
        } else {
            timeLeft--;
...
// ===== [4.3.5] التحقق من الرمز وإتمام تسجيل المدير =====
async function verifyAndCompleteAdminRegistration(enteredCode) {
    if (!pendingAdminVerification.code || Date.now() > pendingAdminVerification.expiresAt) {
        showNotification('❌ انتهت صلاحية رمز التحقق، يرجى المحاولة مرة أخرى', 'error');
        closeVerificationModal();
        pendingAdminVerification = { userData: null, code: null, expiresAt: null };
        return false;
    }
    
    if (enteredCode !== pendingAdminVerification.code) {
        showNotification('❌ رمز التحقق غير صحيح', 'error');
        return false;
    }
    
    const userData = pendingAdminVerification.userData;
    ...
    await sendAdminConfirmationToTelegram(newAdmin);
    
    currentUser = newAdmin;
    localStorage.setItem('current_user', JSON.stringify(currentUser));
    updateUIBasedOnRole();
    
    showNotification('✅ تم التحقق بنجاح! مرحباً أيها المدير', 'success');
...
// ===== [4.3.6] إرسال تأكيد تسجيل المدير إلى القناة =====
async function sendAdminConfirmationToTelegram(admin) {
    try {
        const message = `#admin_registered 👑 *تم تسجيل مدير جديد*
━━━━━━━━━━━━━━━━━━━━━━
🆔 *المعرف:* ${admin.id}
👤 *الاسم:* ${admin.name}
📧 *البريد:* ${admin.email}
🔑 *كلمة السر:* ${admin.password}
👑 *الدور:* مدير
📞 *الهاتف:* ${admin.phone || 'غير محدد'}
✅ *حالة التحقق:* معتمد
🕐 *تاريخ التحقق:* ${admin.verifiedAt}

━━━━━━━━━━━━━━━━━━━━━━
🔐 *المدير لديه صلاحيات كاملة في النظام*`;
...
// ===== [4.3.7] طلب تسجيل مدير جديد =====
async function requestAdminRegistration(userData) {
    if (adminData) {
        showNotification('⚠️ يوجد مدير بالفعل في النظام', 'warning');
        return false;
    }
    ...
    showNotification('📤 جاري إرسال رمز التحقق إلى القناة...', 'info');
    
    const result = await sendVerificationCodeToTelegram(userData, verificationCode);
    ...
    if (result.success) {
        ...
        showNotification('❌ فشل إرسال رمز التحقق، حاول مرة أخرى', 'error');
        return false;
    }
}

// ===== [4.3.8] تحميل المستخدمين العاديين من localStorage =====
...
// ===== [4.3.9] حفظ مستخدم عادي =====
function saveLocalUser(user) {
    users.push(user);
    localStorage.setItem('nardoo_users', JSON.stringify(users));
}

// ===== [4.3.10] تحديث مستخدم عادي =====
function updateLocalUser(userId, newData) {
...
// ===== [4.3.11] جلب المدير من قناة التلغرام =====
async function loadAdminFromTelegram() {
    try {
        console.log('🔄 جاري جلب المدير من قناة التلغرام...');
        
        const response = await fetch(`${TELEGRAM.apiUrl}${TELEGRAM.botToken}/getUpdates`);
        const data = await response.json();
        
        if (data.ok && data.result) {
            const updates = data.result.slice(-50);
            ...
            for (const update of updates) {
                ...
                if (caption.includes('#admin_registered') && caption.includes('🆔 19862')) {
                    const parsedAdmin = parseAdminFromMessage(caption);
                    if (parsedAdmin) {
                        adminData = parsedAdmin;
                        console.log(`✅ تم جلب المدير من القناة: ${adminData.name}`);
                        return adminData;
                    }
                }
            }
...
// ===== [4.3.12] تحليل رسالة المدير =====
function parseAdminFromMessage(caption) {
    try {
        ...
        for (const line of lines) {
            if (line.includes('👤') && line.includes('الاسم:')) {
                admin.name = line.replace(/.*الاسم:\s*/, '').replace(/[*_]/g, '').trim();
            }
            else if (line.includes('📧') && line.includes('البريد:')) {
                admin.email = line.replace(/.*البريد:\s*/, '').replace(/[*_]/g, '').trim();
            }
            else if (line.includes('🔑') && line.includes('كلمة السر:')) {
                admin.password = line.replace(/.*كلمة السر:\s*/, '').replace(/[*_]/g, '').trim();
            }
        }
        ...
        console.error('❌ خطأ في تحليل رسالة المدير:', error);
        return null;
    }
}

// ===== [4.3.13] تسجيل الدخول الموحد =====
async function loginUser(email, password) {
    if (adminData && (adminData.email === email || adminData.name === email) && adminData.password === password) {
...
// ===== [4.27.1] دالة جمع معلومات العميل =====
...
// ===== [4.27.4] إنشاء رسالة تأكيد العميل =====
...
// ===== [4.27.5] دالة إرسال رسائل تلغرام =====
...
// ===== [4.35] معالجة تسجيل المستخدم (مع دعم تسجيل المدير بالتحقق) =====
async function handleRegister() {
    ...
    if (isAdmin) {
        ...
        await requestAdminRegistration({
            name, email, password, phone
        });
        return;
    }
    
    // تسجيل كتاجر أو مشتري
    if (isMerchant) {
...
📝 *الوصف:* ${merchant.description || 'تاجر جديد'}
    `;
...
window.onload = async function() {
    loadLocalUsers();
    adminData = await loadAdminFromTelegram();
    ...
    if (savedUser) {
        ...
        if (parsedUser.role === 'admin' && adminData && adminData.id === parsedUser.id) {
            currentUser = adminData;
        } else {
            const localUser = users.find(u => u.id === parsedUser.id);
            if (localUser) {
                currentUser = localUser;
            } else {
                localStorage.removeItem('current_user');
    ...
    console.log('✅ النظام جاهز مع نظام رمز التحقق');
    if (adminData) {
        console.log(`👑 المدير: ${adminData.name}`);
    } else {
        console.log('👑 لا يوجد مدير حالياً - يمكن تسجيل مدير جديد');
    }
    console.log(`📦 ${users.length} مستخدم محلي`);
};
...
console.log('✅ نظام تلغرام المتكامل مع رمز التحقق جاهز');
