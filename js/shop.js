...
const CartSystem = {
    ...
    // تحديد منطقة التوصيل بناءً على العنوان
    detectZone(address) {
        const zones = {
                        ...
                        <div id="customerFormFields">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="customerAddress" 
                                          ...
                                          class="form-control"
                                          onchange="CartSystem.updateShippingPrice()"></textarea>
                            </div>
    ...
    // تحديث سعر التوصيل بناءً على العنوان
    updateShippingPrice() {
    ...
    populateCheckoutModal() {
                ...
                return `
                    <div class="checkout-item">
                        ...
                        <div class="checkout-item-details">
                            ...
                            <div class="checkout-item-merchant" style="font-size: 11px; color: #D4AF37; margin-top: 3px;">
        ...
        try {
            if (typeof Auth !== 'undefined' && Auth.currentUser) {
                ...
                const formFields = document.getElementById('customerFormFields');
    ...
    async completeOrder() {
        ...
        Utils.showNotification('📦 جاري إرسال الطلب...', 'info');
        
        // إنشاء طلب في نظام التوصيل (إذا كان موجوداً)
        let deliveryId = null;
        if (typeof Delivery !== 'undefined' && Delivery.createDelivery) {
            try {
                ...
                console.log('✅ تم إنشاء طلب في نظام التوصيل:', deliveryId);
            } catch(e) {
                console.warn('⚠️ فشل إنشاء طلب في نظام التوصيل:', e);
            }
        }
        
        // إرسال رسائل منفصلة لكل تاجر
        let merchantMessages = [];
        
        for (const [merchantId, group] of Object.entries(merchantGroups)) {
            // تحديث المخزون لكل تاجر (إذا كان النظام موجوداً)
            if (typeof Inventory !== 'undefined' && Inventory.decreaseStock) {
                for (const item of group.items) {
                    try {
                        ...
                        console.warn(`⚠️ فشل تحديث مخزون ${item.name}:`, e);
                    }
                }
            }
            ...
            // فتح واتساب لكل تاجر (سيتم فتحها في تبويبات منفصلة)
            ...
            if (merchantPhone) {
                ...
                await this.sleep(300); // انتظار بين الفتحات
            }
        }
        
        // إنشاء رسالة رئيسية للإدارة
        ...
        // عرض رسالة نجاح
        Utils.showNotification(`✅ تم إرسال طلبك بنجاح! رقم الطلب: ${mainOrderNumber}`, 'success');
    },
    
    // الحصول على رقم هاتف التاجر من معرفه
    getMerchantPhoneFromId(merchantId) {
        // محاولة جلب من نظام المصادقة
        ...
        // رقم افتراضي (رقم الإدارة)
        return CONFIG.phone.replace(/[^0-9]/g, '');
    },
    
    // إنشاء رسالة الطلب للتاجر
    generateMerchantOrderMessage(data) {
        ...
        if (shippingPrice) message += `🚚 *حصة التوصيل:* ${Math.floor(shippingPrice / Object.keys(data.merchantGroups?.length || 1)).toLocaleString()} دج\n`;
        ...
        message += `━━━━━━━━━━━━━━━━━━━━━━\n`;
        message += `✨ *ناردو برو - تجربة تسوق فاخرة* ✨`;
        
        return message;
    },
    
    // إنشاء رسالة الطلب الرئيسية للإدارة
    generateMainOrderMessage(data) {
        ...
        message += `━━━━━━━━━━━━━━━━━━━━━━\n`;
        message += `🆔 *رقم الطلب الرئيسي:* ${orderNumber}\n`;
        if (deliveryId) message += `🚚 *رقم التوصيل:* ${deliveryId}\n`;
        message += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;
        ...
        message += `📦 *تفاصيل الطلب حسب التجار:*\n`;
        message += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;
        
        let merchantIndex = 1;
        for (const [merchantId, group] of Object.entries(merchantGroups)) {
            message += `🏪 *التاجر ${merchantIndex}:* ${group.merchantName}\n`;
            message += `   📋 المنتجات:\n`;
            
            group.items.forEach(item => {
                message += `      • ${item.name} (${item.quantity} × ${item.price} دج) = ${(item.price * item.quantity).toLocaleString()} دج\n`;
            });
        ...
        message += `\n⏰ *تاريخ الطلب:* ${new Date().toLocaleString('ar-DZ')}\n`;
        message += `━━━━━━━━━━━━━━━━━━━━━━\n`;
        message += `📊 *عدد التجار: ${Object.keys(merchantGroups).length}*\n`;
        message += `📦 *عدد المنتجات: ${this.items.reduce((sum, i) => sum + i.quantity, 0)}*\n`;
        message += `✨ *ناردو برو - نظام إدارة الطلبات المتكامل* ✨`;
...
console.log('✅ نظام السلة المتكامل مع تقسيم الطلبات جاهز');
